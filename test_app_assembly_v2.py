...
try:
    ...
    for attr in ["base_url", "_base_url", "client"]:
        if hasattr(llm, attr):
            ...
            if "openrouter" in str(val).lower() or (
                hasattr(val, "base_url") and "openrouter" in str(val.base_url).lower()
            ):
                base_url_found = True
...
try:
    ...
    prod_tools.register_local_tool(
        RunSqlTool(sql_runner=prod_runner), access_groups=["admin", "user"]
    )
...
if PG_AVAILABLE:
    try:
        ...
        pg_tools.register_local_tool(
            RunSqlTool(sql_runner=pg_sql_runner), access_groups=["admin", "user"]
        )
        pg_tools.register_local_tool(
            VisualizeDataTool(), access_groups=["admin", "user"]
        )
...
try:
    ...
    print(
        f"      Attributes: app={has_app_direct}, create_app={has_create_app}, get_app={has_get_app}"
    )
...
print(
    f"PostgreSQL: {'available' if PG_AVAILABLE else 'not available (PG tests skipped)'}"
)
