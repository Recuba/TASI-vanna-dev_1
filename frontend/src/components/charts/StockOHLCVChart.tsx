...
function exportCSV(data: OHLCVData[], ticker: string, period: string) {
  ...
  a.href = url;
  a.download = `${ticker}_${period}_ohlcv.csv`;
  a.click();
...
function StockOHLCVChartInner({
  ...
  const handleScreenshot = useCallback(() => {
    ...
    a.download = `${ticker}_${period}_${dateStr}.png`;
    a.click();
  }, [ticker, period]);
  ...
  if (!data || data.length === 0) {
    return (
      <ChartEmpty
        height={chartHeight}
        message={`No OHLCV data available for ${stockName || ticker}`}
      />
    );
  }
...
export default dynamic(() => Promise.resolve(StockOHLCVChartInner), {
  ssr: false,
